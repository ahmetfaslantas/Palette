...
const upload = multer({
    storage: multer.diskStorage({
        destination: (req, file, cb) => {
            let path = `./uploads/students/${req.res.locals.userId}/`;
        ...
        filename: (req, file, cb) => {
            ...
            let path = `./uploads/students/${req.res.locals.userId}/${file.originalname}`;
            let name = file.originalname;
            while (fs.existsSync(path)) {
                path = `./uploads/students/${req.res.locals.userId}/${filename}_${edition}.${extension}`;
